...
namespace BurstLibNoise
{
    internal static class Utils
    {
        ...
        internal const float Sqrt3 = 1.7320508075688772935f;
        ...
        internal static readonly float[] Randoms =
        {
        ...
        internal static float GradientCoherentNoise3D(float x, float y, float z, int seed)//, QualityMode quality)
        {
            return noise.snoise(new float3(x, y, z) + Utils.RandomOffset(seed));
            // var x0 = x > 0.0f ? (int) x : (int) x - 1;
            // var x1 = x0 + 1;
            // var y0 = y > 0.0f ? (int) y : (int) y - 1;
            // var y1 = y0 + 1;
            // var z0 = z > 0.0f ? (int) z : (int) z - 1;
            // var z1 = z0 + 1;
        ...
        //     {
        //         return (2.0f * (float) Math.IEEERemainder(value, 1073741824.0f)) - 1073741824.0f;
        //     }
        //     if (value <= -1073741824.0f)
        //     {
        //         return (2.0f * (float) Math.IEEERemainder(value, 1073741824.0f)) + 1073741824.0f;
        ...
        internal static float3 RandomOffset(int seed) {
            return new float3(math.hash(new float2(seed, seed)) % 1000, math.hash(new float2(seed, seed)) % 1000, math.hash(new float2(seed, seed)) % 1000);
        }
    }
}